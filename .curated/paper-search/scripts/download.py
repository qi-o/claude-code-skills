...
try:
    ...
    _fetcher = _ScraplingFetcher(auto_match=False)
    USE_SCRAPLING = True
except ImportError:
    USE_SCRAPLING = False


def _http_get(url, timeout=60, allow_redirects=True, stream=False):
    """HTTP GET with Scrapling primary, requests fallback."""
    if USE_SCRAPLING:
        try:
            page = _fetcher.get(url)
            # Return a simple namespace mimicking requests.Response
            class _Resp:
                def __init__(self, p):
                    self.status_code = p.status if hasattr(p, 'status') else 200
                    self.content = p.body if hasattr(p, 'content') else b''
                    self.headers = p.headers if hasattr(p, 'headers') else {}
            return _Resp(page)
        except Exception:
            pass
...
def download_arxiv(paper_id: str, output_dir: str) -> str:
    ...
    print(f"下载中: {pdf_url}")
    response = _http_get(pdf_url, timeout=60)
...
def download_biorxiv(doi: str, output_dir: str) -> str:
    ...
    print(f"下载中: {pdf_url}")
    response = _http_get(pdf_url, timeout=60, allow_redirects=True)
