...
- [ ] Step 1: Setup & Analyze
  ...
  - [ ] 1.2 Analyze content
  - [ ] 1.3 Check existing 鈿狅笍 REQUIRED
- [ ] Step 2: Confirmation 1 - Style & options 鈿狅笍 REQUIRED
...
```
Input 鈫?Preferences 鈫?Analyze 鈫?[Check Existing?] 鈫?[Confirm 1: Style + Reviews] 鈫?Storyboard 鈫?[Review Outline?] 鈫?Prompts 鈫?[Review Prompts?] 鈫?Images 鈫?PDF 鈫?Complete
```
...
```
馃搵 Loaded preferences from [full path]
鈹溾攢 Watermark: [enabled/disabled] [content if enabled]
鈹溾攢 Art Style: [style name or "auto-select"]
鈹溾攢 Tone: [tone name or "auto-select"]
鈹溾攢 Layout: [layout or "auto-select"]
鈹溾攢 Language: [language or "auto-detect"]
鈹斺攢 Character presets: [count] defined
```

**MUST output this summary** so user knows their current configuration. Do not skip or silently load.

**When EXTEND.md Not Found** 鈫?First-time setup:
...
4. Confirm: "鉁?Preferences saved to [path]"
...
### 1.2 Analyze Content 鈫?`analysis.md`
...
5. **Determine language**:
   - If EXTEND.md has `language` 鈫?use it
   - Else if `--lang` option provided 鈫?use it
   - Else 鈫?use detected source language
...
### 1.3 Check Existing Content 鈿狅笍 REQUIRED
...
## Step 2: Confirmation 1 - Style & Options 鈿狅笍
...
|------|-----------|-------------------|
| Student | 澶ч泟 (Nobita) | Japanese boy, 10yo, round glasses, black hair parted in middle, yellow shirt, navy shorts |
| Mentor | 鍝嗗暒A姊?(Doraemon) | Round blue robot cat, big white eyes, red nose, whiskers, white belly with 4D pocket, golden bell, no ears |
| Challenge | 鑳栬檸 (Gian) | Stocky boy, rough features, small eyes, orange shirt |
| Support | 闈欓 (Shizuka) | Cute girl, black short hair, pink dress, gentle expression |

These are the canonical ohmsha-style characters. Do NOT create custom characters for ohmsha unless explicitly requested.

**After generation**:
- If `skip_outline_review` is true 鈫?Skip Step 4, go directly to Step 5
- If `skip_outline_review` is false 鈫?Continue to Step 4
...
- Page-by-page summary (Cover 鈫?P1 鈫?P2...)
...
**After response**:
1. If user wants to edit 鈫?Wait for user to finish editing, then ask again
2. If user confirms 鈫?Continue to Step 5
...
**After generation**:
- If `skip_prompt_review` is true 鈫?Skip Step 6, go directly to Step 7
- If `skip_prompt_review` is false 鈫?Continue to Step 6
...
**After response**:
1. If user wants to edit 鈫?Wait for user to finish editing, then ask again
2. If user wants to regenerate 鈫?Go back to Step 5
3. If user confirms 鈫?Continue to Step 7
...
3. Generate 鈫?`characters/characters.png`
...
- 澶ч泟: Japanese boy, round glasses, yellow shirt, navy shorts...
- 鍝嗗暒A姊? Round blue robot cat, white belly, red nose, golden bell...
...
鉁?analysis.md
鉁?characters.png
鉁?00-cover-[slug].png ... NN-page-[slug].png
鉁?{topic-slug}.pdf
```
...
| Action | Steps |
|--------|-------|
| **Edit** | Update prompt 鈫?Regenerate image 鈫?Regenerate PDF |
| **Add** | Create prompt at position 鈫?Generate image 鈫?Renumber subsequent (NN+1) 鈫?Update storyboard 鈫?Regenerate PDF |
| **Delete** | Remove files 鈫?Renumber subsequent (NN-1) 鈫?Update storyboard 鈫?Regenerate PDF |
