...
### 1.0 Detect & Save Reference Images 鈿狅笍 REQUIRED if images provided

Check if user provided reference images. Handle based on input type:

| Input Type | Action |
|------------|--------|
| Image file path provided | Copy to `references/` subdirectory 鈫?can use `--ref` |
| Image in conversation (no path) | **ASK user for file path** with AskUserQuestion |
| User can't provide path | Extract style/palette verbally 鈫?append to prompts (NO frontmatter references) |
...
- 01-ref-{slug}.png 鉁?(can use --ref)
- 02-ref-{slug}.png 鉁?(can use --ref)
```
...
鈫?Will append to prompt text (not --ref)
```
...
|-------|------------------|------|
| File path | Ask user (1.2) | 鈫?1.2 |
| Pasted content | `illustrations/{topic-slug}/` | 鈫?1.4 |
...
**CRITICAL**: If EXTEND.md not found, MUST complete first-time setup before ANY other questions or steps. Do NOT proceed to reference images, do NOT ask about content, do NOT ask about type/style 鈥?ONLY complete the preferences setup first.
...
| Result | Action |
|--------|--------|
| Found | Read, parse, display summary 鈫?Continue |
| Not found | 鉀?**BLOCKING**: Run first-time setup ONLY ([config/first-time-setup.md](config/first-time-setup.md)) 鈫?Complete and save EXTEND.md 鈫?Then continue |
...
**CRITICAL**: If article uses metaphors (e.g., "鐢甸敮鍒囪タ鐡?), do NOT illustrate literally. Visualize the **underlying concept**.
...
### Q3: Style 鈿狅笍 REQUIRED (ALWAYS ask, even with preferred_style in EXTEND.md)
...
Style selection based on Type 脳 Style compatibility matrix (styles.md).
Full specs: `styles/<style>.md`

### Q4: Image Text Language 鈿狅笍 REQUIRED when article language 鈮?EXTEND.md `language`
...
|-----|----------|-------------------|
| 01 | 01-ref-diagram.png | direct 鈫?Illustration 1, 3 |
| 02 | 02-ref-chart.png | palette 鈫?Illustration 2 |
```
...
### 5.3 Process References 鈿狅笍 REQUIRED if references saved in Step 1.0
...
1. **VERIFY files exist first**:
   ...
   ```
   - If file MISSING but in frontmatter 鈫?ERROR, fix frontmatter or remove references field
   - If file exists 鈫?proceed with processing
...
- Illustration 1: using 01-ref-brand.png (direct) 鉁?
- Illustration 2: extracted palette from 02-ref-style.png 鉁?
```
...
Positions:
- 01-xxx.png 鈫?After "[Section]"
- 02-yyy.png 鈫?After "[Section]"
