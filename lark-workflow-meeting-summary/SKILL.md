...
version: 1.0.0
description: "会议纪要整理工作流：汇总指定时间范围内的会议纪要并生成结构化报告。当用户需要整理会议纪要、生成会议周报、回顾一段时间内的会议内容时使用。触发词（中文）：整理会议纪要、会议周报、汇总会议纪要、会议总结、回顾会议、生成会议报告。触发词（英文）：meeting summary, summarize meetings, meeting report, meeting minutes roundup, weekly meeting summary."
metadata:
  requires:
    bins: ["lark-cli"]
---
