...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| TRADEOFF 类清理 | 清理 npm cache、pip cache、uv cache 等需重新下载的缓存时 | 明确列出删除后果（重新下载时间），让用户逐项确认 |
| 大型缓存清理 | 清理 torch/huggingface cache（可能数 GB）时 | 提醒重新下载模型可能需要数小时，确认用户不需要离线使用 |
| Docker 操作 | 任何涉及 Docker volume 或 image 的清理时 | 严格禁止自动执行 prune，仅展示数据由用户决定 |
| PowerShell 脚本执行 | 首次运行 .ps1 脚本前 | 展示脚本内容摘要，确认脚本来源可信 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| 管理员权限不足 | Access denied 删除系统文件夹 | 标记为需要管理员权限，提示用户以管理员身份运行 |
| 脚本执行超时 | 分析脚本运行超过 5 分钟 | 增大 timeout 至 600000ms，或跳过该分析项继续其他项 |
| 文件被占用 | Remove-Item 报错文件正在使用 | 提示关闭占用程序（如 IDE、Docker），或跳过该项 |
| PowerShell 内联命令失败 | 复杂命令返回解析错误 | 改为写入 .ps1 文件后用 `-File` 参数执行 |
