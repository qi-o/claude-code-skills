...
```bash
python scripts/generate_schematic.py \
  "MAPK signaling pathway: EGFR receptor 鈫?RAS 鈫?RAF 鈫?MEK 鈫?ERK 鈫?nucleus. \
...
```bash
python scripts/generate_schematic.py \
  "IoT system block diagram: sensors (bottom) 鈫?microcontroller 鈫?\
   WiFi module and display (middle) 鈫?cloud server 鈫?mobile app (top). \
...
鉁?"Make a diagram" (too vague)

### Include Quantitative Details
鉁?"Neural network: input (784), hidden (128), output (10)"  
鉁?"Flowchart: n=500 screened, n=150 excluded, n=350 randomized"  
鉁?"Some numbers" (not specific)
...
鉁?"Color-code by function: blue=input, green=processing, red=output"
...
- 鉁?**Fast**: Results in minutes
- 鉁?**Easy**: Natural language descriptions (no coding)
- 鉁?**Quality**: Automatic review and refinement
- 鉁?**Universal**: Works for all diagram types
- 鉁?**Publication-ready**: High-quality output immediately
