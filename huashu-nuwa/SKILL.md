...
github_hash: ea4b9abfe0e60e5051ed946c70e51434402d3545
version: 1.1.0
---
...
一个好的Skill，让你用另一个人的眼睛看自己的问题。不是为了模仿他们，而是为了拓展你自己的思维边界。
...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 调研质量确认 | Phase 1.5 所有 Agent 完成后 | 展示调研摘要表格，确认信息覆盖度是否满足需求 |
| 提炼结果确认 | Phase 2.5 框架提炼完成后 | 展示心智模型数量和名称，确认方向是否正确 |
| 冷门人物警告 | Phase 0.5 检测到可用来源 <10 条 | 明确告知 Skill 质量受限，降低期望后再继续 |
| 更新 vs 新建 | 用户要求「更新XX的skill」 | 确认是增量更新（仅刷新新信息）还是完全重建 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| Agent 搜索超时 | 5 分钟无有价值结果 | 不等待，继续推进；在诚实边界中标注「信息不足」 |
| Agent 结果冲突 | 多个 Agent 报告矛盾信息 | 保留矛盾，记录到「内在张力」section |
| 信息源匮乏 | 可用来源 <10 条 | 降低心智模型数量至 2-3 个，加大诚实边界篇幅 |
| Phase 4 验证不通过 | 质量自检有 FAIL 项 | 回到 Phase 2 迭代，最多循环 2 次后交付标注局限的版本 |
