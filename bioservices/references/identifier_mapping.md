...
#### Gene Name 鈫?UniProt (via search, then mapping)
...
### Pattern 1: Gene Symbol 鈫?Multiple Database IDs
...
### Pattern 2: Compound Name 鈫?All Database IDs
...
def safe_batch_mapping(ids, from_db, to_db, chunk_size=100):
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        try:
            ...
            print(f"鉁?Processed {min(i+chunk_size, len(ids))}/{len(ids)}")

        except Exception as e:
            print(f"鉁?Error at chunk {i}: {e}")
...
def multi_hop_mapping(gene_symbol, organism="9606"):
    """Gene symbol 鈫?UniProt 鈫?KEGG 鈫?Pathways."""
    u = UniProt()
    k = KEGG()

    # Step 1: Gene symbol 鈫?UniProt
    query = f"gene:{gene_symbol} AND organism:{organism}"
    result = u.search(query, frmt="tab", columns="id")
