# DOCX Skill — Detailed Reference

## Table of Contents

1. [Creating New Documents — Full Code Reference](#creating-new-documents--full-code-reference)
2. [Editing Existing Documents — XML Reference](#editing-existing-documents--xml-reference)
3. [Critical Rules Summary](#critical-rules-summary)
4. [Dependencies](#dependencies)

---

## Creating New Documents — Full Code Reference
...
Landscape orientation: docx-js swaps width/height internally, so pass portrait dimensions:
```javascript
size: {
...
## Editing Existing Documents — XML Reference
...
Auto-repair fixes: `durableId` >= 0x7FFFFFFF, missing `xml:space="preserve"` on `<w:t>` with whitespace.
...
| Rule | Detail |
|------|--------|
| Set page size explicitly | docx-js defaults to A4; use 12240×15840 for US Letter |
| Landscape: pass portrait dimensions | docx-js swaps width/height; pass short edge as `width` |
...
| Replace entire `<w:r>` elements | When adding tracked changes, replace the whole block |
| Preserve `<w:rPr>` formatting | Copy original run's formatting into tracked change runs |
| Use "Claude" as author | For tracked changes and comments, unless user requests otherwise |
