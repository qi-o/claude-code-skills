...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| HIL 决策选择 | FORCE 或 SUGGEST 触发审核门 | 展示审查报告和问题列表，让用户选择 skip/modify/rollback/accept |
| 回滚操作 | 用户选择 rollback | 确认回滚目标（上一版本快照），警告回滚后当前修改将丢失 |
| 批准低分章节 | 建议触发且用户选择 accept | 确认接受当前质量，记录决策原因 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| InkOS audit 失败 | `inkos audit` 返回错误或超时 | 检查 book-id 是否正确；直接读取审查日志文件获取结果 |
| 审查维度缺失 | 审查报告缺少预期维度 | 检查 `book_rules.md` 中 webnovelCraft 配置是否启用 craft 维度 |
| 修改后未改善 | `inkos revise` 后重新审查分数未提升 | 分析失败维度，提供具体修改建议而非通用 polish；考虑回滚到前一版本 |
