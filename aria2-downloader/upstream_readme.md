...
In some cases, you don't want special characters such as 涓? spaces, or &, such as when transferring the downloaded filename to a Windows system or the filename through an 8bit-unsafe channel. In these cases, add the `--restrict-filenames` flag to get a shorter title.
...
youtube-dl test video ''_盲鈫潟?webm    # All kinds of weird characters
...
#### nhkradirulive (NHK 銈夈仒銈嬧槄銈夈仒銈?LIVE)
...
# 鈩癸笍 See help(yt_dlp.YoutubeDL) for a list of available options and public functions
ydl_opts = {}
with yt_dlp.YoutubeDL(ydl_opts) as ydl:
    info = ydl.extract_info(URL, download=False)

    # 鈩癸笍 ydl.sanitize_info makes the info json-serializable
...
ydl_opts = {
    ...
    # 鈩癸笍 See help(yt_dlp.postprocessor) for a list of available Postprocessors and their arguments
...
# 鈩癸笍 See "progress_hooks" in help(yt_dlp.YoutubeDL)
def my_hook(d):
...
# 鈩癸笍 See help(yt_dlp.postprocessor.PostProcessor)
...
with yt_dlp.YoutubeDL() as ydl:
    # 鈩癸笍 "when" can take any value in yt_dlp.utils.POSTPROCESS_WHEN
