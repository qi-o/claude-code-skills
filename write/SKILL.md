...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 意图不明确 | 用户描述模糊，无法匹配路由表中的单一 Skill | 提供分类选项让用户选择，不自行猜测 |
| 多需求并存 | 用户一次提出多个写作需求 | 列出所有识别到的需求，请用户确定优先级和处理顺序 |
| 路由目标不存在 | 路由表匹配到的 Skill 未安装或缺失 SKILL.md | 告知用户该 Skill 不可用，建议替代方案 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| 路由表过时 | 新增 Skill 未出现在路由表中 | 执行 `ls ~/.claude/skills/` 扫描目录，与路由表做 diff，提示更新路由表 |
| 目标 Skill 执行失败 | 被路由的 Skill 返回错误 | 将错误信息转达给用户，建议检查目标 Skill 的前置条件 |
| 用户需求超出范围 | 路由表无匹配项且不属于写作领域 | 明确告知能力边界，列出可处理的写作类别供参考 |
| 语言检测错误 | 中文用户被路由到仅支持英文的 Skill | 在路由前确认用户语言偏好，提供语言适配提示 |
