...
- POST body 为**单个表达式**，不是语句块。直接返回表达式的求值结果
- **不要用 IIFE** `(() => {...})()` — 会返回 `{"error":"Uncaught"}`。直接写表达式即可
- **不要用 `var`/`let`/`return`** — 这些是语句不是表达式
- 支持 `awaitPromise`：可以写 async 表达式
- 返回值必须是可序列化的（字符串、数字、对象），DOM 节点不能直接返回，需要提取属性
- 提取列表数据时用 `JSON.stringify(Array.from(...).map(...))` 包裹，确保返回完整字符串
- 需要复杂逻辑时，用三元运算符或 `.map().filter().reduce()` 链式调用组合成单个表达式
...
# ✅ 提取列表
curl -s -X POST "http://localhost:3456/eval?target=ID" -d 'JSON.stringify(Array.from(document.querySelectorAll("a")).slice(0,10).map(a=>({text:a.textContent,href:a.href})))'
```

## 截图注意

- 后台 tab（通过 `/new` 创建）可能因为未被 Chrome 渲染而导致 `Page.captureScreenshot` 超时
- 遇到超时时：优先用 `/eval` 提取数据替代截图；或在已有可见 tab 中操作
- Windows 环境下 `file` 参数路径使用 Unix 风格（`/c/Users/...`）
