...
未通过时引导用户完成设置：
- **Node.js 缺失**：提示用户安装 Node.js 22+（https://nodejs.org）
- **Chrome 未开启远程调试**：引导用户在 Chrome 地址栏打开 `chrome://inspect/#remote-debugging`，勾选 **"Allow remote debugging for this browser instance"**，可能需要重启浏览器
...
如果前置检查输出的 site-patterns 列表中有匹配的站点，必须读取对应文件获取先验知识（平台特征、有效模式、已知陷阱）。经验内容标注了发现日期，当作可能有效的提示而非保证——如果按经验操作失败，回退通用模式并更新经验文件。
...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 登录态不足 | 目标内容无法获取且判断登录可解决时 | 告知用户在 Chrome 中登录目标网站，等待确认后继续 |
| 反爬风险操作 | 短时间内需要打开大量页面（>10 个 tab）或执行密集交互时 | 提示账号封禁风险，确认继续或调整策略（降低频率/使用代理） |
| 敏感操作确认 | 需要在用户已打开的 tab 中操作（而非新建后台 tab）时 | 明确说明将要操作的用户 tab 及影响，获取明确许可 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| CDP Proxy 连接失败 | `check-deps.mjs` 报告端口不通或 Proxy 启动超时 | 检查 Chrome remote-debugging 是否开启，引导用户重启 Chrome 并勾选调试选项 |
| 页面反爬拦截 | `/eval` 返回验证码页面、403、或空白内容 | 切换到 GUI 交互模式（手动点击/滚动），或从搜索引擎入口重新导航 |
| DOM 操作超时 | curl 请求 CDP API 超过 10s 无响应 | 关闭当前 tab，新建 tab 重试；若持续失败，尝试 `/navigate` 刷新页面 |
| 目标内容不存在 | 页面显示"内容不存在"但无法确认是否为反爬 | 尝试从站内搜索/推荐链接重新定位，或换用 WebSearch 从外部查找一手来源 |
