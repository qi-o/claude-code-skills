...
description: 深度调研主题并自动生成知识关系图谱PDF。接收研究主题后自动进行网络调研、信息收集、知识整理，最终生成专业的可视化关系图谱。当用户说"研究...并做图"、"深度分析...并可视化"、"生成知识图谱"、"可视化调研"、"研究并可视化"、"生成关系图"、"画关系图"、"knowledge graph"、"research and visualize"、"generate diagram from research"时使用。Do NOT use for text-only reports (use deep-research instead), or when user already has structured data (use structure-to-pdf instead).
license: MIT
version: 0.1.0
metadata:
...
**必需**：
- Graphviz: 自动检测安装位置，支持跨平台
  - macOS: `brew install graphviz`
  - Linux: `sudo apt-get install graphviz`
  - Windows: `winget install Graphviz.Graphviz` 或从 https://graphviz.org/download/ 下载
...
本 Skill 包含自动检测 Graphviz 安装位置的功能：

- 自动检测操作系统（Windows、macOS、Linux）
- 首先检查 PATH 环境变量
- 如果 PATH 中未找到，搜索常见安装位置：
...
- Windows 下使用 PowerShell 搜索 Program Files

**注意**：即使 Graphviz 未在 PATH 中，Skill 也能自动找到并使用它。
...
**问题：找不到 Graphviz**
- 症状：错误消息 "Graphviz is not installed or not found"
- 解决方案：
  1. 确认 Graphviz 已安装
  2. 如果使用 Windows，确保安装路径正确
  3. Skill 会自动搜索常见位置，一般无需手动配置

**问题：dot 命令不在 PATH 中**
- 症状：系统提示找不到 dot 命令
- 解决方案：本 Skill 会自动查找 Graphviz，无需添加到 PATH

**问题：Windows 下 Graphviz 安装后仍找不到**
- 可能原因：安装路径不在默认位置
- 解决方案：
  1. 确认安装路径（通常是 `C:\Program Files\Graphviz\`）
  2. Skill 会使用 PowerShell 搜索 Program Files
  3. 如果安装在其他位置，手动添加到 PATH
...
- **v1.1** (2026-01-02): 改进 Graphviz 检测
  - 添加跨平台 Graphviz 自动检测功能
  - 支持 Windows、macOS、Linux 自动查找
  - Windows 下使用 PowerShell 搜索 Program Files
  - 改进错误处理和文件锁定问题
  - 默认同时生成 PDF 和 PNG 格式
  - 更新故障排除文档
...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 调研范围确认 | 主题较宽泛可能产生大量节点 | 确认核心范围和边界，建议聚焦子主题 |
| 图谱结构设计 | 完成调研后选择可视化方案 | 展示候选结构（人物关系/概念图谱/技术架构），确认方向 |
