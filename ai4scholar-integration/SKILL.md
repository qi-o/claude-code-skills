...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 批量论文下载 | 调用 read_* 下载 5+ 篇论文全文 | 确认论文列表和存储目录，避免消耗大量时间和磁盘 |
| auto_cite 引用标注 | 调用 auto_cite 工具为长文本添加引用 | 确认引用格式（IEEE/APA/Vancouver/Nature），标注后需人工审核 |
| sci_draw 绘图 | 调用 sci_draw 生成科研图 | 确认绘图描述和输出格式（PNG/SVG） |
| DOI 付费下载 | 通过 DOI 下载需要机构订阅的论文 | 提示可能需要校园网/机构访问，确认是否有 VPN |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| API Key 不足/过期 | ai4scholar 工具返回认证错误 | 提示用户访问 ai4scholar.net 获取新 Key；回退到 paper-search skill |
| 论文全文不可获取 | read_* 返回空或 404 | 尝试其他数据库（arXiv/Semantic Scholar）；回退到仅获取摘要 |
| 搜索结果为空 | search_* 返回 0 条结果 | 扩大关键词范围，移除年份限制，或切换数据库 |
| auto_cite 匹配不准确 | 引用标注位置与内容不匹配 | 提示用户人工审核和调整，提供 BibTeX 供手动修改 |