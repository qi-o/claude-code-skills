...
```
Structure
  鈹斺攢鈹€ Model       (multiple models for NMR structures)
      鈹斺攢鈹€ Chain   (e.g., chain A, B, C)
          鈹斺攢鈹€ Residue  (amino acids, nucleotides, heteroatoms)
              鈹斺攢鈹€ Atom (individual atoms)
```
...
print(f"Distance: {distance:.2f} 脜")
```
...
for residue_key in dssp:
    ...
    print(f"Residue {residue_id}: {ss}, 蠁={phi:.1f}掳, 蠄={psi:.1f}掳")
```
...
nearby_atoms = ns.search(center_atom.coord, 5.0)  # 5 脜 radius
print(f"Found {len(nearby_atoms)} atoms within 5 脜")
