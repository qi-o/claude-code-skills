...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 目标格式选择 | 用户未指定文献格式（IEEE/APA/Vancouver/Nature） | 确认目标格式，避免格式化后返工 |
| 摘要重写范围 | 五要素诊断发现 2+ 要素缺失/模糊 | 确认是否允许重写对应段落，还是仅提供修改建议 |
| 换投期刊规则 | 用户提供的投稿指南与默认规范有冲突 | 确认以投稿指南为准还是保留部分默认格式 |
| 文献信息缺失 | DOI 或出版信息无法确认 | 确认标注 `[待核实]` 还是跳过该条目 |
...
| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| 期刊缩写无法匹配 | 英文期刊名无标准缩写记录 | 保留全称并标注 `[缩写待核实]` |
| 用户提供的文献信息不完整 | 缺少作者/年份/卷号/页码等关键字段 | 标注缺失字段，跳过格式化该条目，列出需补充的信息 |
| 摘要超出字数限制 | 修改后中文 >300 字或英文 >250 词 | 截断至限制并提示用户自行调整优先级 |
| LaTeX 三线表语法错误 | 生成的 booktabs 代码缺少必需包或环境 | 提供完整的 `\usepackage{booktabs}` 提示和最小可编译示例 |
