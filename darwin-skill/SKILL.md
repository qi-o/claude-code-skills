...
language: HTML
dependencies: []
tags: [skill-optimization, quality, evaluation, autonomous]
---
...
| 检查点 | 触发条件 | 确认内容 |
|--------|---------|---------|
| 基线评估完成后 | Phase 1 完成评分卡 | 展示评分卡和短板分析，确认后再进入优化循环 |
| 每个 skill 优化后 | Phase 2 单个 skill 完成一轮改进 | 展示改动摘要和分数变化，用户确认 OK 才继续 |
| 探索性重写 | hill-climbing 连续瓶颈，提议从头重写 | 明确告知会丢弃当前版本，需用户同意才执行 |
| 回滚操作 | 新版本得分低于旧版本 | 自动 git revert，但需告知用户回滚原因 |

## 错误处理与回退

| 错误场景 | 检测信号 | 回退策略 |
|---------|---------|---------|
| 子 agent 超时 | 测试 prompt 执行超时 | 退化为干跑验证（dry_run），在 results.tsv 中标注 |
| Git 分支冲突 | git commit 或 revert 失败 | 检查工作区状态，手动解决冲突后继续 |
| SKILL.md 膨胀 | 优化后文件大小超过原始 150% | 触发约束规则 4，放弃本轮改进并回滚 |
| 评分退化 | 新总分 <= 旧总分 | 自动 git revert HEAD 回滚，记录失败尝试到 results.tsv |
