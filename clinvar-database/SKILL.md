...
description: |
  ...
  触发场景：
  (1) 用户需要查询基因变异的临床意义、致病性分类
  (2) 用户说"变异致病性"、"ClinVar查询"、"基因变异注释"、"VCF注释"、"临床意义"
  (3) 需要注释 VCF 文件中的变异位点
  Do NOT use for pathway analysis (use kegg-database or reactome-database instead).
license: Unknown
metadata:
...
- 鈽呪槄鈽呪槄 Practice guideline - Highest confidence
- 鈽呪槄鈽?Expert panel review (e.g., ClinGen) - High confidence
- 鈽呪槄 Multiple submitters, no conflicts - Moderate confidence
- 鈽?Single submitter with criteria - Standard weight
- 鈽?No assertion criteria - Low confidence

**Critical considerations:**
- Always check review status - prefer 鈽呪槄鈽?or 鈽呪槄鈽呪槄 ratings
...
5. Consult expert panel classifications (鈽呪槄鈽? when available
...
1. Search using web interface or E-utilities:
   ...
   ```
2. Review results, noting review status (should be 鈽呪槄鈽?or 鈽呪槄鈽呪槄)
