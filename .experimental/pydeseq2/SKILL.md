...
# 1. Load data
counts_df = pd.read_csv("counts.csv", index_col=0).T  # Transpose to samples 脳 genes
...
**Input requirements:**
- **Count matrix:** Samples 脳 genes DataFrame with non-negative integer read counts
- **Metadata:** Samples 脳 variables DataFrame with experimental factors

**Common data loading patterns:**

```python
# From CSV (typical format: genes 脳 samples, needs transpose)
...
1. **Data orientation matters:** Count matrices typically load as genes 脳 samples but need to be samples 脳 genes. Always transpose with `.T` if needed.
