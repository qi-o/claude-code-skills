...
Count data typically comes in genes 脳 samples format but needs to be transposed:

```python
import pandas as pd

# Load count matrix (genes 脳 samples)
counts_df = pd.read_csv("counts.csv", index_col=0)

# Transpose to samples 脳 genes
counts_df = counts_df.T

# Load metadata (already in samples 脳 variables format)
metadata = pd.read_csv("metadata.csv", index_col=0)
```
...
- 鉁?Ensure counts are non-negative integers
- 鉁?Verify samples 脳 genes orientation
- 鉁?Check that sample names match between counts and metadata
- 鉁?Remove or handle missing metadata values
- 鉁?Filter low-count genes (typically < 10 total reads)
- 鉁?Verify experimental factors are properly encoded
