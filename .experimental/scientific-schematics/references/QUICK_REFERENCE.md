...
**How it works:** Describe your diagram 鈫?Nano Banana Pro generates it automatically
...
```bash
python scripts/generate_schematic.py \
  "MAPK pathway: EGFR 鈫?RAS 鈫?RAF 鈫?MEK 鈫?ERK" \
  -o mapk.png
```
...
```bash
python scripts/generate_schematic.py \
  "Op-amp circuit with 1k惟 resistor and 10碌F capacitor" \
  -o circuit.png
```
...
- "MAPK signaling: receptor 鈫?RAS 鈫?RAF 鈫?MEK 鈫?ERK 鈫?nucleus, label each phosphorylation"
...
- 鉁?No coding required
- 鉁?No templates needed
- 鉁?No manual drawing
- 鉁?Automatic quality review
- 鉁?Publication-ready output
- 鉁?Works for any diagram type
