...
3. **Decision**: If quality >= threshold 鈫?**DONE** (no more iterations needed!)
...
- 鉁?Saves API calls if first generation is good enough
- 鉁?Higher quality standards for journal papers
- 鉁?Faster turnaround for presentations/posters
- 鉁?Appropriate quality for each use case
...
鉁?**Good prompts** (specific, detailed):
- "CONSORT flowchart showing participant flow from screening (n=500) through randomization to final analysis"
- "Transformer neural network architecture with encoder stack on left, decoder stack on right, showing multi-head attention and cross-attention connections"
- "Biological signaling cascade: EGFR receptor 鈫?RAS 鈫?RAF 鈫?MEK 鈫?ERK 鈫?nucleus, with phosphorylation steps labeled"
- "Block diagram of IoT system: sensors 鈫?microcontroller 鈫?WiFi module 鈫?cloud server 鈫?mobile app"

鉁?**Avoid vague prompts**:
...
- 鉁?Layout and composition
- 鉁?Labels and annotations
- 鉁?Colors and styling
- 鉁?Quality review and refinement
- 鉁?Publication-ready output
...
鈹? 1. Generate image with Nano Banana Pro             鈹?
鈹?                   鈫?                               鈹?
鈹? 2. Review quality with Gemini 3 Pro                鈹?
鈹?                   鈫?                               鈹?
鈹? 3. Score >= threshold?                             鈹?
鈹?      YES 鈫?DONE! (early stop)                      鈹?
鈹?      NO  鈫?Improve prompt, go to step 1            鈹?
鈹?                   鈫?                               鈹?
鈹? 4. Repeat until quality met OR max iterations      鈹?
...
**Example:**
- For a **poster** (threshold 7.0): Score of 7.5 鈫?**DONE after 1 iteration!**
- For a **journal** (threshold 8.5): Score of 7.5 鈫?Continue improving
...
```
鉁?"Flowchart with vertical flow, top to bottom"
鉁?"Architecture diagram with encoder on left, decoder on right"
鉁?"Circular pathway diagram with clockwise flow"
```

**2. Include Quantitative Details:**
```
鉁?"Neural network with input layer (784 nodes), hidden layer (128 nodes), output (10 nodes)"
鉁?"Flowchart showing n=500 screened, n=150 excluded, n=350 randomized"
鉁?"Circuit with 1k惟 resistor, 10碌F capacitor, 5V source"
```

**3. Specify Visual Style:**
```
鉁?"Minimalist block diagram with clean lines"
鉁?"Detailed biological pathway with protein structures"
鉁?"Technical schematic with engineering notation"
```

**4. Request Specific Labels:**
```
鉁?"Label all arrows with activation/inhibition"
鉁?"Include layer dimensions in each box"
鉁?"Show time progression with timestamps"
```

**5. Mention Color Requirements:**
```
鉁?"Use colorblind-friendly colors"
鉁?"Grayscale-compatible design"
鉁?"Color-code by function: blue for input, green for processing, red for output"
```
